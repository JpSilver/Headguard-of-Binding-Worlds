...
namespace MS3D{
	...
	inline glm::mat3x4 createBoneMatrix(vec3 rotation, vec3 position) {
		vec4 quat = vec4(0, 0, 0, 1);
		glm::mat3x4 returnMat;
		Math::QuaternionMatrix(quat, returnMat);
		returnMat[0][3] = position.x;
		returnMat[1][3] = position.y;
		returnMat[2][3] = position.z;
		return returnMat;
	};

	inline glm::mat3x4 createBaseMatrix(vec3 rotation, vec3 position) {
		m_matrix returnMat;
		Math::AngleMatrix(rotation, returnMat);
		returnMat[0][3] = position.x;
		returnMat[1][3] = position.y;
		returnMat[2][3] = position.z;
		glm::mat3x4 retMat = Math::convertm_Matrix(returnMat);
		return retMat;
	};
