...
int main(int argc, char *argv[])
{
	...
	Armature::_DeltaTime = 0;

	shader = Shader("shaders/SimpleShader.vert", "shaders/SimpleShader.frag");
	Armature arm = modelLoader.loadMS3dAnimation("Data/Skelitons/Char.animation");
	...
	while (!quit)
	{
		...
		while (SDL_PollEvent(&sdlEvent) != 0)
		{
			...
			if (keys[SDL_SCANCODE_W]) {
				camera.move(vec3(0.0F, 0.0F, speed*Armature::_DeltaTime));
			}
			if (keys[SDL_SCANCODE_S]) {
				camera.move(vec3(0.0F, 0.0F, -speed*Armature::_DeltaTime));
			}
			if (keys[SDL_SCANCODE_A]) {
				camera.move(vec3(-speed*Armature::_DeltaTime, 0.0F, 0.0F));
			}
			if (keys[SDL_SCANCODE_D]) {
				camera.move(vec3(speed*Armature::_DeltaTime, 0.0F, 0.0F));
			}
			if (keys[SDL_SCANCODE_Q]) {
				camera.move(vec3(0.0F, speed*Armature::_DeltaTime, 0.0F));
			}
